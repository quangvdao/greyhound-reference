...
int polcomprf_serialize(uint8_t *out, size_t outlen, const polcomprf *pi) {
  ...
  if(!out || outlen != polcomprf_serialized_size(pi)) return 1;
  memcpy(out,"GHPR",4);
...
size_t polcomprf_contextual_serialized_size(const polcomprf *pi) {
  ...
  if(pi->cpp->kappa1 > (SIZE_MAX-8)/(N*QBYTES)) return 0;
  return 8+pi->cpp->kappa1*N*QBYTES;
}

int polcomprf_serialize_contextual(uint8_t *out, size_t outlen,
                                   const polcomprf *pi) {
  if(!out || outlen != polcomprf_contextual_serialized_size(pi)) return 1;
  put64(out,pi->normsq);
  polzvec_bitpack(&out[8],pi->u2,pi->cpp->kappa1);
  return 0;
}
...
int polcomprf_deserialize_contextual(polcomprf *pi, const polcomprf *context,
  ...
  if(!kappa1 || kappa1 > (SIZE_MAX-8)/(N*QBYTES)) return 2;
  size=8+kappa1*N*QBYTES;
